<script lang="ts">
	...
	// Extrait l'année depuis la date de sortie "YYYY-MM-DD"
	const year = $derived(movie.release_date ? movie.release_date.slice(0, 4) : '');
	// Note TMDB sur 10, arrondie à 1 décimale. Affiche "—" si pas encore notée.
...
>
	...
	>
		{#if posterUrl}
			...
			/>
		{:else}
			<!-- Fallback quand TMDB n'a pas d'affiche pour ce film -->
